            ...
            var url = "/todo/"+tid+"/data";
         	$.get(url).done(function(data){
                  ...
                  if (data["task"]["status"] == 0){
         			   $('#status').append('<td><select name="status" class="inputDisabled"><option value="0" selected="selected">Approved</option><option value="1">Disapproved</option></select></td>');
                  }
                  if (data["task"]["status"] == 1){
         			   $('#status').append('<td><select name="status" class="inputDisabled"><option value="0" >Approved</option><option value="1" selected="selected">Disapproved</option></select></td>');
                  }
		
                  if (data["task"]["category"] == 0){
         			   $('#category').append('<td><select name="category" class="inputDisabled"><option value="0" selected="selected">Entertainment</option><option value="1">Meals</option></select></td>');
                  }
                  if (data["task"]["category"] == 1){
         			   $('#category').append('<td><select name="category" class="inputDisabled"><option value="0" >Entertainment</option><option value="1" selected="selected">Meals</option></select></td>');
                  }
